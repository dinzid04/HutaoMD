...
const syncLidDatabase = (store) => {
    if (!store || !store.contacts) return;
    // Hanya sync sekali saja untuk menghindari looping berat berulang kali
    if (isLidSynced) return;

    // console.log(chalk.cyanBright(`[INIT] 🔄 Syncing LID Database from Store...`));
    let added = 0;
    try {
        for (const [realJid, data] of Object.entries(store.contacts)) {
            if (data.lid && !lidDatabase[data.lid]) {
                lidDatabase[data.lid] = { jid: realJid };
                added++;
            }
        }
        if (added > 0) {
            saveLidDatabase();
            console.log(chalk.green(`[INIT] ✅ Synced ${added} new LIDs from Store.`));
        }
        isLidSynced = true;
    } catch (e) {
        console.error('Error syncing LID database:', e);
    }
};
...
const resolveJid = (jid, store, metadata) => {
    ...
    // 0. Sync if needed
    if (!isLidSynced && store) {
        syncLidDatabase(store);
    }
  ...
  if (m.key) {
    ...
    if (rawSender.endsWith('@lid') && !lidDatabase[rawSender]) {
        ...
        if (lidDatabase[rawSender] && lidDatabase[rawSender].jid) {
             console.log(chalk.cyanBright(`[AUTO LEARN - PC] 💾 Ditemukan setelah Sync (Private Chat)!\nMenyimpan: ${rawSender} => ${lidDatabase[rawSender].jid}`));
        } else {
             // Jika masih tidak ketemu setelah sync, berarti memang tidak ada di kontak.
